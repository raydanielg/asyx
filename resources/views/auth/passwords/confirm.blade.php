...
<div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
    ...
    <div class="bg-gradient-to-br from-amber-600 to-amber-700 px-8 py-8 text-center">
        ...
        <p class="text-amber-100 text-sm mt-1">Please confirm to continue</p>
    </div>

    {{-- Form --}}
    <div class="p-8">
        <p class="text-sm text-gray-600 mb-5">This is a secure area. Please confirm your password before continuing.</p>
        ...
        <form method="POST" action="{{ route('password.confirm') }}" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
                <button type="submit" class="py-3 px-6 text-sm font-bold text-white bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 rounded-lg shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Confirm
                </button>

                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="text-sm font-medium text-amber-600 hover:text-amber-700 transition-colors">Forgot password?</a>
                @endif
            </div>
        </form>
    </div>
